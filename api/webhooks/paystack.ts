...
export default withMethods({
  POST: async (req: VercelRequest, res: VercelResponse) => {
    ...
    const signatureValid = await paystackGateway.validateWebhook({
    ...
    let event;
    try {
      event = paystackGateway.parseWebhook(payload);
    } catch (err) {
      logger.error('Failed to parse Paystack webhook', { error: String(err) });
    ...
    if (isDuplicate) {
      logger.info('Duplicate Paystack webhook — skipping', { gatewayEventId: event.gatewayEventId });
      res.status(200).json({ received: true, duplicate: true });
      return;
    }

    if (!signatureValid) {
      logger.warn('Paystack webhook signature invalid', { gatewayEventId: event.gatewayEventId });
      await markEventProcessed(storedEvent.id, 'Signature validation failed');
      res.status(200).json({ received: true }); // 200 to prevent Paystack retries on signature mismatch
      return;
    }

    if (event.type !== 'PAYMENT_SUCCESS') {
      logger.info('Paystack webhook: non-success event type', { type: event.type });
    ...
    try {
      ...
      logger.error('Paystack webhook processing error', { error: processingError, orderReference: event.orderReference });
    }

    await markEventProcessed(storedEvent.id, processingError);

    // Always return 200 — errors are recorded; Paystack should not retry on logic failures
    res.status(200).json({ received: true });
  },
});
